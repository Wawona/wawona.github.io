...
**Correction (2026-08-14).** This post is the Feb 2026 intro. Product facts now: CalVer (not older 0.x tags). The full Apple family plus Android and Linux. VMs and containers are Machine types (forbidden on tvOS/watchOS). Desktop and LockScreen are coming soon on macOS and Android; iOS only via repo.wawona.io (not the App Store app). anowaW is a separate planned app bridge. Graphics libs live in `wwn-*` flake inputs. See [docs](/docs/) and [FAQ](/faq/).
